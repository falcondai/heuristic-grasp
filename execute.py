...
workspace_low = np.asarray([0.62, 0.62, -0.02])
workspace_high = np.asarray([0.72, 0.72, -0.02])
...
x = a * workspace_low[0] + (1. - a) * workspace_high[0]
y = b * workspace_low[0] + (1. - b) * workspace_high[0]
# execute_linear(left, x, y, workspace_high[2], Quat.from_v_theta([0,0,1], np.random.rand() * 2. * np.pi), n_steps=1)
...
for i in xrange(int(sys.argv[1])):
    ...
    execute_planar_grasp(left, left_gripper, z_initial, z_table, n_steps=10)
    execute_linear(left, 0.6, 0.7, z_initial, Quat.from_v_theta([0,0,1], np.random.rand() * 2. * np.pi), n_steps=1)
